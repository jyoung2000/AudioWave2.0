...
source (HTMLMediaElement / buffer)
  ...
  → retune (AudioWorkletNode "np-pitch-shifter", ratio 0.5–2.0, or bypass)
...
- **Latency**: `baseLatency` + `outputLatency` (when the browser exposes it) + worklet grain length (≈ 2048 samples at 48 kHz ≈ 43 ms) are measured/estimated by `measureLatency()` and reported to group mode for compensation and to the Audio settings page.
