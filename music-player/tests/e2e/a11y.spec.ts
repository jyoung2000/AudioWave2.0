...
const SECTIONS = ['Music', 'Now playing', 'Up next', 'Playlists', 'Search', 'Constellation', 'Listening', 'Equaliser', 'Settings'] as const;
...
test('@a11y every section is reachable with the keyboard alone', async ({ page }) => {
  ...
  await expect(page.getByRole('option', { name: /Settings/i })).toBeFocused();
});
...
test('@a11y respects reduced motion', async ({ page }) => {
  ...
  await page.reload();
  await page.getByRole('option', { name: /Constellation/i }).click();
...
test('@a11y the empty library state is announced as a group with a name', async ({ page }) => {
  await page.getByRole('option', { name: /^Music/i }).click();
