...
export function localFileReport(): { active: boolean; features: LocalCapability[] } {
  ...
  return {
    ...
    features: [
      ...
      { name: 'The equaliser and the retune worklet', available: workletDataUrl() !== null, note: workletDataUrl() !== null ? 'The worklet is carried inside this file and loaded from memory.' : 'This build has no worklet, so retuning falls back to changing playback speed and says so.' },
      ...
      { name: 'Group listening with a hub', available: false, note: 'Group listening needs a WebSocket, which a browser refuses to open from a local file. Use the served player for that.' },
